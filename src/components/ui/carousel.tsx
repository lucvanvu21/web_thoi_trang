...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselDotsProps>(
  ({ className, thumbnails, height, width, ...props }, ref) => {
      ...
      >
        {thumbnails.map((thumb, index) => (
          <Button
            key={index}
            className={clsx(
              height ? `md:h-24` : ' md:h-14  md:w-20',
              width ? `md:w-20` : 'md:h-14  md:w-20',

              'embla__dot rounded-3xl h-10 w-10 overflow-hidden md:rounded-lg border-4 p-0',
